...
{
    // ES3C28P 无旋钮，直接返回静止态，避免对无效引脚 digitalRead。
    if (PIN_EC06_A < 0 || PIN_EC06_B < 0) return 0;
...
{
    // ES3C28P 无 MCP23017 扩展按键（MODE/PREV/NEXT 原为 MCP 输入），
    // 这些引脚在 keys_pins.h 里定义为负值哨兵，直接视为"未按下"。
...
// 把触摸手势映射到现有按键语义（按当前 UI 上下文分发）。
// ES3C28P 以触摸屏为主要交互方式。
static void handle_touch_events()
{
  TouchEvent ev;
  while ((ev = touch_poll()).gesture != TouchGesture::NONE) {
    // NFC 绑定类型弹窗（旋钮/上下选择，单击确认，长按/双击取消）
    if (s_nfc_bind_popup_active) {
      switch (ev.gesture) {
        case TouchGesture::TAP:        nfc_bind_popup_confirm(); break;
        case TouchGesture::SWIPE_UP:   nfc_bind_popup_move(+1); break;
        case TouchGesture::SWIPE_DOWN: nfc_bind_popup_move(-1); break;
        case TouchGesture::DOUBLE_TAP:
        case TouchGesture::LONG_PRESS:
        case TouchGesture::SWIPE_LEFT:
        case TouchGesture::SWIPE_RIGHT: nfc_bind_popup_close(); break;
        default: break;
      }
      continue;
    }

    // 列表选择模式：单击确认，双击/长按退出，上下移动，左右翻页
    if (player_list_select_is_active()) {
      switch (ev.gesture) {
        case TouchGesture::TAP:        list_select_key_and_refresh(KEY_PLAY_SHORT); break;
        case TouchGesture::DOUBLE_TAP:
        case TouchGesture::LONG_PRESS: list_select_key_and_refresh(KEY_MODE_LONG); break;
        case TouchGesture::SWIPE_UP:   list_select_key_and_refresh(KEY_PREV_SHORT); break;
        case TouchGesture::SWIPE_DOWN: list_select_key_and_refresh(KEY_NEXT_SHORT); break;
        case TouchGesture::SWIPE_LEFT: list_select_key_and_refresh(KEY_PAGE_DOWN_SHORT); break;
        case TouchGesture::SWIPE_RIGHT: list_select_key_and_refresh(KEY_PAGE_UP_SHORT); break;
        default: break;
      }
      continue;
    }

    // 快捷菜单：单击确认，双击/长按/右滑退出，上下移动，左滑返回
    if (quick_menu_is_active()) {
      switch (ev.gesture) {
        case TouchGesture::TAP:        quick_menu_key_and_refresh(QuickMenuKey::Confirm); break;
        case TouchGesture::DOUBLE_TAP:
        case TouchGesture::LONG_PRESS: quick_menu_key_and_refresh(QuickMenuKey::Exit); break;
        case TouchGesture::SWIPE_UP:   quick_menu_key_and_refresh(QuickMenuKey::Up); break;
        case TouchGesture::SWIPE_DOWN: quick_menu_key_and_refresh(QuickMenuKey::Down); break;
        case TouchGesture::SWIPE_LEFT: quick_menu_key_and_refresh(QuickMenuKey::Back); break;
        case TouchGesture::SWIPE_RIGHT: quick_menu_key_and_refresh(QuickMenuKey::Exit); break;
        default: break;
      }
      continue;
    }
    ...
    if (g_app_state == STATE_NFC_ADMIN) {
      switch (ev.gesture) {
        case TouchGesture::TAP: nfc_admin_state_on_key(NFC_ADMIN_KEY_PLAY_SHORT); break;
        case TouchGesture::DOUBLE_TAP:
        case TouchGesture::LONG_PRESS: nfc_admin_state_on_key(NFC_ADMIN_KEY_MODE_SHORT); break;
        default: break;
      }
      continue;
    }

    // 播放器页（默认）：单击播放/暂停，双击/左滑下一曲，右滑上一曲，
    // 上滑/长按进快捷菜单，下滑进列表浏览。
    switch (ev.gesture) {
      case TouchGesture::TAP:        play_key_toggle_with_solenoid(); break;
      case TouchGesture::DOUBLE_TAP:
      case TouchGesture::SWIPE_LEFT: player_next_track(); break;
      case TouchGesture::SWIPE_RIGHT: player_prev_track(); break;
      case TouchGesture::SWIPE_UP:
      case TouchGesture::LONG_PRESS: enter_quick_menu_from_player(); break;
      case TouchGesture::SWIPE_DOWN: player_next_group(); break;
      default: break;
    }
  }
}
